...
of a spec that IS diffable, so this re-renders it and compares the result
against the committed file byte for byte. The art files are pinned to LF in
.gitattributes, because a checkout that rewrote them would fail that comparison
and would also change the sha256 this receipt records.
...
def check_seed_is_recorded(_unused: list[Path]) -> list[str]:
    ...
    svg = ART_LIB.header_svg(
        {"name": "chorus", "role": "x", "tagline": "y", "words": []})
    stamp = f"SEED {ART_LIB.seed_for('chorus') % 100000:05d}"
